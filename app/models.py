...
class Pessoa(Base):
    ...
    pessoa_fisica = relationship("Pessoa_fisica", back_populates="pessoa", uselist=False, cascade="all, delete-orphan")
    enderecos     = relationship("Endereco",      back_populates="pessoa", cascade="all, delete-orphan")
    emails        = relationship("Email",         back_populates="pessoa", cascade="all, delete-orphan")
...
class Pessoa_fisica(Base):
    ...
    def __repr__(self):
        return f"<Pessoa_fisica {self.id} {self.cpf}>"
...
class Endereco(Base):
    ...
    rua         = Column(String(60), nullable=False)
    ...
    def __repr__(self):
        return f"<Endereco {self.id} {self.cep}>"
...
class Telefone(Base):
    ...
    id                   = Column(Integer, primary_key=True, autoincrement=True)
    telefone_celular     = Column(String(11), nullable=True)
